    ...
    public partial class MainDashboard : Form
    {
        ...
        public void getdata()
        {
            ...
            try
            {
                conn1.Open();
                int pendding = 0;
                string query1 = "select count(status)  from tbl_incidentdata where status=''";
                ...
                int ongoings = 0;
                string query3 = "select count(status)  from tbl_incidentdata where status='ONGOING'";
                MySqlCommand cmd3 = new MySqlCommand(query3, conn1);
                ongoings = Convert.ToInt32(cmd3.ExecuteScalar());
                ongoing.Text = ongoings.ToString();

                int completedd = 0;
                string query2 = "select count(status)  from tbl_incidentdata where status = 'COMPLETE'";
